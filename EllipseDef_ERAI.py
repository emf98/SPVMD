## DEFINITION FILE FOR ERA-I DATASET
...
## Two sets of definitions save for ERA-I data
...
def ERAI_one_ellipse_calc(lev_list,contour_list,windlat,temp_lev,y,m,t_d,
    ...
    for plot_lev,the_contour in zip(lev_list,contour_list): #loop through each level
        ...
        if plot_lev == 10:
            ...
            gfile = xr.open_dataset("YOUR_LOCATION/erai/"+str(year)+"/g."+str(year)+".nc")
            g_files = gfile["g"]
            tfile = xr.open_dataset("YOUR_LOCATION/erai/"+str(year)+"/t."+str(year)+".nc")
            t_files = tfile["t"]
            ufile = xr.open_dataset("YOUR_LOCATION/erai/"+str(year)+"/u."+str(year)+".nc")
            u_files = ufile["u"]
            
            #The next few lines section the ERA-I data.
