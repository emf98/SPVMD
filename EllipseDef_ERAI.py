...
def ERAI_one_ellipse_calc(file_location,plot_lev,the_contour,windlat,y,m,t_d):
    ...
    gfile = xr.open_dataset(str(file_location)+"/gph/erai_gph_"+str(year)+".nc")
    g_files = gfile["z"]
    tfile = xr.open_dataset(str(file_location)+"/t/erai_t_"+str(year)+".nc")
    t_files = tfile["t"]
    ufile = xr.open_dataset(str(file_location)+"/u/erai_u_"+str(year)+".nc")
    u_files = ufile["u"]
    ...
    return rat, wind, cenlat, cenlon, sz, ep, num; 