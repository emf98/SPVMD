...
## two sets of definitions save for ERA5 data
# The first definition statement averages across metrics in instances of multiple child vortices.
# The second definition statement saves the geometries of multiple vortices separately.
...
#empty lists to append geometries (rat10,wind10,cenlat10,cenlon10,size10,ep10,numb10,rat50,wind50,cenlat50,cenlon50,size50,ep50,numb50) 
...
def ERA5_one_ellipse_calc(lev_list,contour_list,windlat,y,m,t_d,
    ...
    for plot_lev,the_contour in zip(lev_list,contour_list): #loop through each level
        ...
        ##the next line creates a text file for the diagnostics, if desired
        #text_file2 = "./"+date1.strftime("%Y%m%d_%H%M")+"_temp"+str(plot_lev)+".txt"
        #fout2 = open(text_file2,'w')

        #Initiate with first pressure level. 
        #10hPa is used here. Please modify this based on the first value in your lev_list. 
        
        if plot_lev == 10:
            ###THESE LINES MUST BE CHANGED FOR WHERE YOUR DATA IS STORED#####
            gfile = xr.open_dataset("YOUR_LOCATION/era5_gph_"+str(year)+".nc")
            g_files = gfile["z"]
            tfile = xr.open_dataset("YOUR_LOCATION//era5_t_"+str(year)+".nc")
            t_files = tfile["t"]
            ufile = xr.open_dataset("YOUR_LOCATION//era5_uv_"+str(year)+".nc")
            u_files = ufile["u"]
        ...
        for date in date_list: #loop through each day 
            ...
            for d in ind:
                t = d
                height = g_data.loc[dict(time=times[t],level=plot_lev)] 
                height = height/9.81 ##convert to hPa, gph
                u = u_data.loc[dict(time=times[t],level=plot_lev)].mean(dim='latitude') #take zonal-mean
            ...
            for isoline in isoline_list:
                ...
                ##Add line to the text file, if needed.
        ...
        #Add the values to the empty lists provided by the definition statement.   
        if plot_lev == 10:
            print('lev 10')
            rat10[:] = rat[:]
            wind10[:] = wind[:]
...
def ERA5_two_ellipse_calc(lev_list,contour_list,windlat,y,m,t_d,
                     rat10,wind10,cenlat10,cenlon10,size10,ep10,numb10,
                     rat50,wind50,cenlat50,cenlon50,size50,ep50,numb50):
    ...
    for plot_lev,the_contour in zip(lev_list,contour_list): #loop through each level
        ...
        #TWO here to account for the separate vortices. 
        rat = []
        cenlt = []
        cenln = []
        wind = []
